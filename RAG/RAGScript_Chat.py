...
if __name__ == "__main__":
    ...
    if st.session_state.logged_in and not st.session_state.finished_logging:
        ...
        system_content_retr = "It is your Task to generate only a single SQL Query for the underlying Database, without using the \n character. You must assure that you use only a single semicolon and only return a single string in your response. Names of days must be shortened to the three letter standard. We are currently in the WinterSemester2024_25. Abbreviations are SoSe, SS for SommerSemester and WiSe, WS for WinterSemester. So queries have to be for the next semester SommerSemester2025, unless specified differently. You must only generate queries which select from tables which actually exist within the database structure! Try to mostly use 'WHERE module' as part of your Query, unless specified otherwise. Do not use 'WHERE ects' as part of a query. Only, and only if you are asked for master courses make sure to include '%MP%' or '%MWP%', if you are creating a query over the modules and to concatenate using AND. Only, and only if you are asked for bachelor courses make sure you include '%BP%' or '%BWP%', if you are creating a query over the modules and to concatenate using AND. Do not engage with the topic, only create a SQL Query"
